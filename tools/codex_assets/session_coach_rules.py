...
def usage_notices(codex_home: pathlib.Path, warn_thread_tokens: int) -> tuple[list[Notice], bool]:
    ...
    if not threads:
        return [], False
    notices = thread_notices(threads[0], warn_thread_tokens)
    return notices, any(notice.code in {"THREAD_LONG", "CTX_PRESSURE"} for notice in notices)


def thread_notices(active: Any, warn_thread_tokens: int) -> list[Notice]:
    notices: list[Notice] = []
    ...
    if not active.rollout_path:
        return notices
    return notices + rollout_notices(pathlib.Path(active.rollout_path))


def rollout_notices(rollout_path: pathlib.Path) -> list[Notice]:
...
def asset_notices(groups: dict[str, list[str]]) -> list[Notice]:
    notices: list[Notice] = []
    if groups["skills"]:
        notices.append(make_notice(
    ...
    if groups["scripts"]:
        notices.append(make_notice(
            ...
            ["rtk bash /home/leiwenjun/codex/scripts/session-coach.sh --help", "rtk python3 -m unittest discover -s tests -p 'test_*.py'"],
...
def repo_notices(groups: dict[str, list[str]], phase: str) -> list[Notice]:
    notices = agent_notices(groups) + asset_notices(groups) + delivery_notices(groups, phase)
    if groups["archive"]:
        notices.append(make_notice(
